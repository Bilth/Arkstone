...
namespace BeautifyEffect {
				...
				public enum BEAUTIFY_OUTLINE_MODE {
								Depth = 0,
								Sobel = 1
				}
	...
	public class Beautify : MonoBehaviour {
								...
								public BEAUTIFY_OUTLINE_MODE _outlineMode = BEAUTIFY_OUTLINE_MODE.Depth;

								public BEAUTIFY_OUTLINE_MODE outlineMode {
												get { return _outlineMode; }
												set {
																if (_outlineMode != value) {
																				_outlineMode = value;
																				if (_outlineMode == BEAUTIFY_OUTLINE_MODE.Depth) {
																								_outlineColor.a = 0.8f;
																				} else {
																								_outlineColor.a = 0.35f;
																				}
																				UpdateMaterialProperties ();
																}
												}
								...
								//		[SerializeField]
								//		Texture2D _sunFlareTexture;
								//
								//		public Texture2D sunFlareTexture {
								//			get { return _sunFlareTexture; }
								//			set {
								//				if (_sunFlareTexture != value) {
								//					_sunFlareTexture = value;
								//					UpdateMaterialProperties ();
								//				}
								//			}
								//		}
								...
								public static string SKW_FRAME_MASK = "BEAUTIFY_FRAME_MASK";
								...
								public static string SKW_VIGNETTING_MASK = "BEAUTIFY_VIGNETTING_MASK";
								...
								public static string SKW_BLOOM_USE_DEPTH = "BEAUTIFY_BLOOM_USE_DEPTH";
								public static string SKW_BLOOM_USE_LAYER = "BEAUTIFY_BLOOM_USE_LAYER";
		...
		Material
								...
								const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint50";
								float sunFlareCurrentIntensity;
								Vector4 sunLastScrPos;
								float sunLastRot;
								Texture2D flareNoise;
								RenderTexture dofDepthTexture, dofExclusionTexture, bloomSourceTexture, bloomSourceDepthTexture;
								float sunFlareTime;
								...
								void OnEnable () {
												...
												if (EditorPrefs.GetInt (BEAUTIFY_BUILD_HINT) == 0) {
																...
																EditorUtility.DisplayDialog ("Beautify Update", "Before building your game, please check the Build Options section in Beautify's inspector for new shader capabilities and make sure you disable unused features to optimize build size and compilation time.\n\nOtherwise when you build the game it will take a long time during shader compilation!", "Ok");
												}
#endif
								}
								...
								void CheckDoFTransparencySupport () {
												...
												dofDepthTexture = RenderTexture.GetTemporary (currentCamera.pixelWidth / _depthOfFieldDownsampling, currentCamera.pixelHeight / _depthOfFieldDownsampling, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
								...
								void CheckDoFExclusionMask () {
												...
												depthCam.depthTextureMode = DepthTextureMode.None;
												depthCam.renderingPath = RenderingPath.Forward;
												dofExclusionTexture = RenderTexture.GetTemporary (currentCamera.pixelWidth / _depthOfFieldDownsampling, currentCamera.pixelHeight / _depthOfFieldDownsampling, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
								...
								//		[ImageEffectTransformsToLDR]	 -> use only if Beautify is last effect to use HDR buffers - removed for safety for now
								void OnRenderImage (RenderTexture source, RenderTexture destination) {
												...
												if (renderPass == 0) {
																rtBeauty = RenderTexture.GetTemporary (source.width, source.height, 0, source.format);
												}
												...
												if (_lensDirt || _bloom || _anamorphicFlares || sunFlareEnabled) {
																...
																if (_bloom || (_lensDirt && !_anamorphicFlares)) {
																				...
																				if (_quality == BEAUTIFY_QUALITY.BestQuality && _bloomAntiflicker) {
																								Graphics.Blit (_bloomCullingMask != 0 ? bloomSourceTexture : source, rt [0], bMat, 9);
																				} else {
																								Graphics.Blit (_bloomCullingMask != 0 ? bloomSourceTexture : source, rt [0], bMat, 2);
																				}
																...
																if (_anamorphicFlares) {
																				...
																				if (_anamorphicFlaresAntiflicker && _quality == BEAUTIFY_QUALITY.BestQuality) {
																								Graphics.Blit (_bloomCullingMask != 0 ? bloomSourceTexture : source, rtAF [0], bMat, 9);
																				} else {
																								Graphics.Blit (_bloomCullingMask != 0 ? bloomSourceTexture : source, rtAF [0], bMat, 2);
																				}
																...
																if (sunFlareEnabled) {
																				...
																				if (sunFlareCurrentIntensity > 0) {
																								...
																								bMat.SetColor ("_SunTint", _sunFlaresTint * sunFlareCurrentIntensity); //  sunLastScrPos.z = sunFlareCurrentIntensity;
																								...
																								RenderTexture rtSF = RenderTexture.GetTemporary (currentCamera.pixelWidth / _sunFlaresDownsampling, currentCamera.pixelHeight / _sunFlaresDownsampling, 0, rtFormat);
																								int sfRenderPass;
																								if (_quality == BEAUTIFY_QUALITY.BestQuality) {
								...
								void UpdateMaterialProperties () {
												...
												bMat.SetFloat ("_OutlineMode", _outlineMode == BEAUTIFY_OUTLINE_MODE.Depth ? 1f : 0f);
